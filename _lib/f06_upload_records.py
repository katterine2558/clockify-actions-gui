...
from _lib.upload_timeentries import upload_timeentries
import tkinter

def open_upload_records_window(menu_window, images_path):
    ...
    window.title("Cargar horas")
    window.after(201, lambda: window.iconbitmap(os.path.join(images_path, "Subir_horas.ico")))
    ...
    def click_upload_records():
        ...
        if datos == {}:
            ...
            if "Proyecto" not in list(datos.keys()) or "Descripción" not in list(datos.keys()) or "Tarea" not in list(datos.keys()) or "Correo electrónico" not in list(datos.keys()) or "Fecha de inicio" not in list(datos.keys()) or "Hora de inicio" not in list(datos.keys()) or "Fecha de finalización" not in list(datos.keys()) or "Hora de finalización" not in list(datos.keys()):
                ...
                upload_timeentries(datos,workspace_id)
    ...
    upload_button = CTkButton(
        ...
        width=100,
        height=30,
        command=click_upload_records
    )
    upload_button.place(x=100, y=165)
